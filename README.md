...
| --- | --- | --- | --- |
| **`/omj`** | Gather specs + author an implementation spec (Plan) + recommend one execution lane, then stop (read-only primer) | The starting point for every FE task | `/omj https://figma.com/design/abc?node-id=1-2 /settings/profile` |
| **`/omj-start`** | Handoff an approved OMJ spec to the selected OMC/OMX execution lane | After approving a spec when auto-start is unavailable | `/omj-start ./omj-search-spec.md` |
...
> **read-only vs active op.** `/omj` and `/omj-review` are read-only (report only) — `/omj` may ask one post-spec execution-lane question, but still cannot Write/Edit/build/test. `/omj-start` is a handoff command: it launches only when the runtime surface is explicit and safe, otherwise it prints one copyable action. `/omj-verify`, `/omj-fix`, and `/omj-sync` (sync/push) are active ops using Figma write / `Edit` / Bash; if your environment blocks those in Plan mode, exit Plan mode first. Each command's syntax, arguments, and steps live in its `commands/<name>.md` (the source of truth).
...
- **Plan-native primer**: `/omj` is read-only — it drafts a spec, asks at most one execution-lane selector, and stops; implementation starts only after you approve (ExitPlanMode).
