...
| **`/oh-my-joy:fix`** | Fix defects on a route (required) from a pasted screenshot and/or a complaint, then re-capture to confirm (active loop). Supports `--base <url>` (dev server) and `--commit` (commit the confirmed fix) | Quick fixes for pixel/visual defects | `/oh-my-joy:fix /pricing "banner z-index too low"` |
...
- **Token-guard hooks** — `check-design-tokens.mjs` (hardcoded-color warning) and `check-story-exists.mjs` (missing-Story warning) in `templates/hooks/`. **The plugin never fires them by itself** — they run only after `/oh-my-joy:setup` copies and registers them into a consuming project's `.claude/hooks/` (opt-in), and they no-op without an `.omj/fe-context.md` declaration. Both are **advisory by convention — fail-open**: even an unexpected crash exits 0, so a hook defect can never block your session (pinned by [`tests/hooks/hook-conventions.test.mjs`](tests/hooks/hook-conventions.test.mjs)).
- **OMJ HUD statusline** — a vendored statusLine HUD in `hud/` (attribution and license in [`NOTICE.md`](NOTICE.md); layout, known limitations, and bundle regeneration in [`hud/README.md`](hud/README.md)). **Opt-in like the hooks**: `/oh-my-joy:setup` copies `hud/` to `~/.claude/omj-hud/` (user-global, not per-project) and registers the `statusLine` in `~/.claude/settings.json` only on consent; reruns announce an update when the plugin canon differs from the copy.
...
- `/oh-my-joy:sync extract <figma-url>` — extract all Figma variables into CSS custom properties (`/`→`-` naming, primitive→semantic `var()` references preserved, mapping table written to your project's `docs/design-tokens.md`). Bootstraps projects with no token store yet.
...
- `~/.claude/omj-hud/` (plus a `statusLine` entry in `~/.claude/settings.json`) — user-global, only when you opt into the OMJ HUD during `/oh-my-joy:setup`.
...
- **Spec format**: uSpec sections (Anatomy / Structure / Color·Tokens / Props·Variants / A11y / Motion) + FF 4-criteria + a11y + Figma fidelity ([`figma-fidelity.md`](skills/frontend-fundamentals/references/figma-fidelity.md)) per item.
...
- **Suspect a stale install?** Every release records the tagged tree's content hash in its GitHub Release notes, and CI re-verifies the tag against that hash after publishing. Recompute a local copy's hash with `node scripts/generate-inventory.mjs --dir <plugin cache dir>` — a matching `sha256` means the install is exactly what was released.
