...
- **본문 절차에 호출 지점이 없는 도구는 선언하지 않는다.** Bash는 실행 가능한 최소 prefix까지 좁힌다(`Bash(npm:*)`가 아니라 `Bash(npm i -g playwright-cli:*)`). 위험한 실행을 굳이 사전승인하지 않는 것 자체가 안전 게이트다 — 권한 프롬프트가 사용자 확인 지점이 된다(PRINCIPLES ③).
