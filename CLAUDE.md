...
oh-my-joy(마켓플레이스 `omj`)는 **코드↔Figma 프론트엔드 루프 전체**를 다루는 독립 Claude Code 플러그인이다 — ① design→code(퍼블리싱) ② 시각검증 ③ code→Figma 토큰 sync. OMC(oh-my-claudecode)와 별개이며 같이 써도 무충돌(네임스페이스 `/omj*` vs `/omc-*`). 멘탈 모델: **"FE는 무조건 /omj로 시작 — 커지면 승인 후 OMC executor로 escalate. 백엔드/범용/리서치만 OMC 직접."** `/omj` 스펙은 사용자의 OMC 실행 도구(`/ralph` 순차·`/team` 병렬·`/goal` 장기, 계획은 `/omc-plan`·`/ralplan`)가 소비하는 입력이 된다 — 상세는 README "OMJ × OMC 통합 작업 플로우".

## 문서화 규율 (최우선)

기능 추가/변경 시 아래 셋을 **반드시 함께 갱신**한다(코드만 바꾸고 문서를 안 고치면 미완료):

1. **README**(사용법) — 커맨드/플래그/동작 변경 반영.
2. **CHANGELOG**(항목) — 변경 1건 = 1항목.
3. **docs/PRINCIPLES.md**(동작 원리) — 원리·설계 결정·멘탈 모델이 바뀌면 갱신.

정본 사실(커맨드명·설치 문자열 `/plugin install oh-my-joy@omj`)은 **모든 문서에서 일치**시킨다. 멘탈 모델 등 **서술형은 README를 정본**으로 두고 다른 문서는 요약/링크만 한다(verbatim 복제를 강제하지 않음 — 드리프트 방지).

## 커맨드 규칙

- 네이밍: 모든 커맨드는 `/omj-*` 접두(루트 `/omj` 제외).
- v1 커맨드: `/omj`(Plan 프라이머)·`/omj-review`(FF 코드 diff 리뷰)·`/omj-verify`·`/omj-fix`(시각 결함 수정 루프, 능동)·`/omj-sync`·`/omj-setup`(의존성 닥터). (v1.1: `/omj-push`·`/omj-spec`.)
...
- **code→Figma는 "코드가 이김" 단방향**(드리프트만 보고).
