...
- The README opens with the surface in tiers — what you type (`deep-interview` when fuzzy → `spec` → approve → `ship`), what the plan runs for you (`review` → `verify` → `fix`), occasional tools (`sync`, `setup`), and the internal agents that are never typed — and the Commands table carries a Tier column
- Re-review rules in `/oh-my-joy:review` — a second pass on the same change opens with a `Prior findings` table (resolved / unresolved / regressed), reviews only the hunks that changed, demotes a new finding on unchanged code to a note unless it says why it was not visible before, never worsens a severity on unchanged code once the earlier blockers are resolved, reports work that grew past the spec as one `scope expansion` finding, and after a third pass with an unresolved blocker declares that the review is not converging
- Execution rules in `docs/EXECUTION-HANDOFF.md` — between approval and the report the session asks no questions (the plan is the answer set; assumptions are recorded where it is silent); a blocker is `resolvable` until three distinct approaches have failed, and only a `human-only` blocker (credentials, an external approval, a physical step, a paid resource) stops the run at once; review and verify run against the same diff. The completion-procedure line printed by both primers carries the rule; `tests/standalone.test.mjs` pins the canon
- Evidence kinds in `/oh-my-joy:verify` evidence mode — every row records `test report`, `command replay`, or `browser capture` next to its exit code, and a route recorded in the spec is reported as still owed when only evidence mode ran
- An ambiguity floor in `/oh-my-joy:deep-interview` — the reported score cannot drop below what the ledger shows open (disputed facts, unscored dimensions, assumed answers); plus a rhythm guard (three confirmation questions in a row are followed by an assumption-exposing one), no "shall I continue?" after an ordinary round, and `Research first` rows for dimensions only a measurement can score
- Eval cases `spec-critique-gate` and `review-rerun-delta` (with the `review-changes-pass2` fixture), a `## Critique` grader on both spec cases, cheap regex graders for the simulated tasks naming real files and for the prior-findings status column, and an evidence-kind requirement on `verify-evidence-mode`; `docs/EVALS.md` lists the cases by command and records which loops are interactive-only
- The fallback eval runner saves every run's final message and grader results (`results/<stamp>/<case>/run-N.md` and `run-N.json`, linked from the aggregate as `outputPath`), checks the cost ceiling before a run starts against an estimate (`--run-cost-estimate`, default 2, then the case's previous run) and refuses to start when the budget is below one run, retries an unparseable judge reply once and reports a second failure as an unscored grader (`aggregates.judgeRetries`, `judgeFailures`), and accepts `OMJ_EVAL_CLAUDE_BIN` / `OMJ_EVAL_TMPDIR` so `tests/eval-runner.test.mjs` can drive it against a stub `claude` without spending tokens

### Changed

- The `figma-implementer` agent is renamed `implementer` and gains a general mode: frontend mode for specs with uSpec sections, Figma nodes, or a route, general mode for everything else. It is the executor on the inline lane and the teammate type for every Dispatch row, so the approved-spec requirement and the evidence contract now hold for non-frontend rows too; it explores before editing, spawns no further agents, asks nothing mid-run, classifies blockers, and reports decisions with their assumptions
- A run the eval runner started is always graded, judge calls included; the ceiling no longer skips llm graders after a run overshoots it. `.github/workflows/evals.yml` runs one round per case under a $15 ceiling by default and takes a `runs` input; `docs/EVALS.md` and both READMEs record the measured cost per run and the one-case, one-run local loop
- The read-only tier is "no write tools" rather than "no subagents": `spec` and `review` declare `Agent` for the `critic` agent only, and `tests/plugin-manifest.test.mjs` pins that every command declaring `Agent` names `critic` in its body and that `critic` declares exactly `Read`, `Grep`, `Glob`; `deep-interview` declares `Skill` to hand its requirements to `spec`. The bundled agents are three (`implementer`, `design-qa`, `critic`)
- `docs/PRINCIPLES.md` — ① carries "critique before consent" in two readings and the three gates in sequence (interview → spec → approval), ③ the relaxed tier and its two test pins, ④ drops the subagent argument, ⑤ the re-review ratchet, ⑦ the execution rules, ⑪ three bounded question classes all before approval with the exit bridge as the interview's last round, ⑧ a v0.9.0 decision table (a separate critique command declined because a command shares the session context; the four-role roster adopted in part as one `critic` agent; a settings surface deferred; anything needing a runtime declined); the README gains "The spine at a glance", the two-entry rule under the tagline, and the surface tiers

### Deprecated

- The agent name `figma-implementer` — use `implementer`; the row stays in the README migration table until the next minor release
...
- The eval runner skipped every llm grader once a single run had overshot `--max-cost-usd`, leaving the run it had paid for unjudged, and scored a judge reply it could not parse as 0 instead of marking it unscored
- `.claude-plugin/plugin.json` and `marketplace.json` descriptions still advertised the commands retired in 0.8.0 (durable goal loop, consensus review) and the pre-0.8 frontend-only framing; both now describe the spec-first spine
