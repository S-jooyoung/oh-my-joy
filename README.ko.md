...
- **`/goal`** — **세션 안**의 끈기: 이 세션이 조건을 만족할 때까지 계속 반복하게 합니다. Claude Code 네이티브 goal 지원(훅 활성 환경)이 필요합니다.
- **agent team** — 독립 병렬 레인 2개 이상으로 쪼개지는 작업(화면 여러 장·문서·검증): 승인된 스펙을 병렬 Claude 서브에이전트에 분배합니다. Claude Code 네이티브 에이전트 팀 지원이 필요합니다.
