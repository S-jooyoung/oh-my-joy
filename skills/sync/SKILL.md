...
- In `sync`, `check`, and `push`, when the Figma integration is disconnected or the active file cannot be read, give the canonical desktop-app, active-tab, and duplicate guidance and stop. `check` then reports only that the Figma side was not read, with no drift counts, classes, or store inventory; `push` reports that nothing was pushed and leaves the store untouched.
