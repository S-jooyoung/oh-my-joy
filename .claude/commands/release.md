...
3. Decide the version: `--version` if given, otherwise `node scripts/release.mjs next` (with `--bump` when passed). If `next` fails — an empty `[Unreleased]` prints "nothing to release" — stop with its message; never create a branch or run the cut with an empty version. Call the result X.Y.Z from here on.
