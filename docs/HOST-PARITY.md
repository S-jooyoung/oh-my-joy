...
OMJ supports both hosts from one repository. Workflow names alone are insufficient: the adapter must preserve the actual operation, approval scope, and evidence. This inventory covers every Claude entry point and the host-specific setup assets as of 2026-09-09.
...
| Host discovery boundary | 10 canonical commands once; the shared rubric remains usable | 10 workflow adapters, 3 internal role adapters, and the shared rubric (14 skills total) | The 13 Codex-only adapters set `user-invocable: false` and `disable-model-invocation: true`; validator and manifest tests pin both flags |
...
Claude discovers plugin commands and skills through separate component paths. The canonical `skills/` root therefore stays in place for Codex, while its 13 host-only adapters use both Claude discovery controls: `user-invocable: false` removes them from the command menu and `disable-model-invocation: true` prevents model selection. The shared `frontend-fundamentals` skill omits both flags. Setting only one flag leaves the other exposure path open; relocating the root would change the existing Codex manifest and relative references, and adding another manifest skills path is additive rather than a replacement. See Claude's [skill frontmatter](https://code.claude.com/docs/en/skills#frontmatter-reference) and [plugin component path](https://code.claude.com/docs/en/plugins-reference#component-path-fields) references.
...
The source compatibility gate used temporary fixtures and did not refresh an installed plugin. Claude Code 2.1.266 showed all 10 canonical workflows exactly once in its real command menu; invoking `deep-interview` with an empty body dispatched the canonical command body byte-for-byte after plugin-root expansion and called no tools. Codex CLI 0.153.4 received the temporary `skills/` tree through the app-server process extra-root mechanism: initialized `skills/list` returned all 14 skills enabled, and native explicit skill inputs for `deep-interview` and `critic` read their linked canonical command and agent bodies successfully. The ignored local evidence is retained under `.omj/goals/claude-command-dedup/evidence`; tracked documentation does not link to that runtime path.

Earlier setup evidence remains valid: local Codex CLI 0.153.4 accepted both maintainer skills through initialized app-server `skills/list` with `forceReload: true`, including the repository-only scope. A non-persistent configuration override was read back with the exact five selected status indicators. Native `hooks/list` also discovered the project handler from a nested cwd and reported it as enabled but untrusted; configuration and project trust do not bypass per-hook trust. No plugin installation, user-global configuration update, or release was needed for these checks.
