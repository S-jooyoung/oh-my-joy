...
Codex exposes the same ten workflow entry points (including the `spec` compatibility alias) as skills. The three internal roles (`critic`, `implementer`, `design-qa`) are used by the workflow and are not normal entry points. Claude-only presentation extras (the OMJ HUD and answer style) remain Claude-only; the workflow, Figma/browser fallbacks, plan gate, durable goals, review, verification, sync, and ship contracts are shared.
...
- **`/oh-my-joy:ship` opened the PR against the wrong branch** — pass `--base <branch>` (teams on `develop` pass `--base develop` every time) or answer the base question it asks without the flag; `gh pr edit <n> --base <branch>` fixes an already-open PR. Running ship while sitting on `develop` with changes is safe: it branches off first instead of committing there.
