...
- **도달 라우트 검증 (캡처를 증거로 쓰기 전 필수)**: `snapshot` 결과의 현재 URL·페이지 컨텍스트가 `"$ROUTE"`에 실제로 도달했는지 확인한다. 리다이렉트로 다른 화면(로그인·홈 등)에 있으면 그 캡처를 기준과 비교하지 말고 **"예상 라우트 미도달(리다이렉트됨)"을 실패로 보고**한다 — 잘못된 화면을 찍고 "일치"로 보고하는 조용한 오검증이 이 커맨드의 가장 위험한 실패 모드다. 재로그인 후에도 미도달이면 여기서 종료한다(무리한 비교 추정 금지).
