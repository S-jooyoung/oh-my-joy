...
- **Baseline comparison not happening** — Figma asset URLs expire after ~7 days; re-run `/omj` to refresh the spec's baseline provenance. Cross-session comparison relies on the PNGs in `.omj/baselines/` (gitignore recommended).
