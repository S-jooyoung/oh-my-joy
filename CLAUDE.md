...
- **HUD** (`hud/`): vendored statusLine HUD — `hud/vendor/hud/index.js` is a generated esbuild bundle (display strings rebranded to OMJ); never hand-edit it — regenerate per `hud/README.md`. Copy-installed user-globally by `/oh-my-joy:setup` like the hooks (opt-in). Attribution: `NOTICE.md`.
