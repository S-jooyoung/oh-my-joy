...
`ralplan` owns the complete implementation plan. It inspects current code and, when relevant, Figma or all paginated GitHub PR review material. The lead planner self-checks every plan, and independent readings are sized to it: none for two or fewer target files with no shape change or risk, one fresh `critic` for three to five, and fresh `architect` and `critic` readings for larger, shape-changing, or risky plans, with a delta pass only while a 🔴 or BLOCK remains and at most two revision rounds. Reviewers advise; the lead records every accepted, rejected, or synthesized disposition and never claims artificial consensus.
...
Evidence is tied to the current repository fingerprint. Change goals use the helper's argv-based `verify`; report goals use `evidence`. Every goal requires a truly independent passing `review`, and closure requires another final review. A review with no 🔴 and no BLOCK is recorded as pass with its 🟡 dispositions; the goal completes on that fingerprint, accepted 🟡 changes land after all goals are complete, and the final review, which receives every goal's acceptance criteria and the full diff, covers them. When the host exposes no independent reviewer context, execution reports that blocker; the lead cannot relabel self-review as independent. Final proof and final review must share the current fingerprint, so an edit after either invalidates closure. A ledger can close only after all goals and PR findings have evidence and any required delivery has been read back.
