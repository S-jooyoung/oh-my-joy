...
- **실행**: `/goal`/`$ultragoal`(durable goal/checkpoint) · `/team`/`$team`(병렬 N에이전트) · `/ralph`/`$ralph`(순차 루프). `/omj-start`는 자동 시작이 안 될 때의 canonical fallback handoff.
...
- `/omj`는 실행 레인 선택을 **한 번만** 묻고, 1번 옵션을 `(추천)`으로 둡니다. 선택값은 최종 스펙에 남깁니다.
