...
- The fail-closed review path (`Review: incomplete`, and ultragoal never recording it as a pass) has no case: the harness cannot remove `Agent` from a run, so a three-file diff always gets its independent pass, and an asynchronous critic handback can leave a trailing message as the graded last message. Before a release, run `/oh-my-joy:review` by hand on a three-file diff in a session without `Agent` and read the saved report for `Review: incomplete`.
...
| `/oh-my-joy:review` | `review-mixed-diff`, `review-rerun-delta`, `review-verification-weakening` | both file classes with severities; a second pass reports prior findings first and only the delta; a skipped test and a loosened assertion are 🔴 `verification weakened` |
