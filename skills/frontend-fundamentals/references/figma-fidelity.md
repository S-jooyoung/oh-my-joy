...
When moving a Figma design into code, **never altering the designer's decisions** is the default. These are project-agnostic universal rules: `/oh-my-joy:ralplan` prescribes them while authoring the plan, and `/oh-my-joy:review`·`design-qa` verify them after implementation (prescription↔verification, same SoT).
